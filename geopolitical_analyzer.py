...
def translate_headline(title: str) -> str:
    """영어 뉴스 제목을 한글로 번역 (완전 매칭 우선, 미매칭 시 원문 반환)

    NOTE: 키워드 치환 방식(PHRASE_MAP/WORD_MAP)은 한/영 혼합 문장을 생성하여
    원문보다 가독성이 낮아지므로 비활성화함. 완전 매칭이 없으면 원문을 그대로 반환.
    """
    if not title:
        return title
    ...
    FULL_TITLE_MAP = {
        ...
        # 2026-03-28 현재 뉴스
        "The Tech Download: How Russia could profit from Iran war helium supply chain dis":
            "테크 다운로드: 러시아가 이란 전쟁으로 인한 헬륨 공급망 차질에서 이익을 얻는 방법",
        "What comes next? Three attack scenarios as U.S. sends thousands more troops to M":
            "다음은? 미국이 중동에 수천 명 추가 파병하는 3가지 공격 시나리오",
        "'Not unlike tariffs': Iran war threatens to deepen Asia private equity's worst f":
            "'관세와 다르지 않다': 이란 전쟁, 아시아 사모펀드 최악의 자금조달 침체 심화 위협",
        "Rubio says US expects to finish Iran war 'in next couple of weeks'":
            "루비오, 미국이 이란 전쟁을 '앞으로 몇 주 내' 종결 예상한다고 발언",
        "Gulf markets are splintering as the Iran war continues. Here's what to know":
            "이란 전쟁 지속에 걸프 시장 분열. 알아야 할 사항",
    }

    # 완전 매칭 시도 (제목 앞 30자 기준, 대소문자 무시)
    title_stripped = title.strip()
    title_lower = title_stripped.lower()
    for eng_title, kr_title in FULL_TITLE_MAP.items():
        if title_lower.startswith(eng_title[:30].lower()):
            return kr_title

    # 매칭 없음 → 원문 반환
    # (PHRASE_MAP/WORD_MAP 키워드 치환은 한/영 혼합 문장을 생성해 원문보다 읽기 어려워 사용 중단)
    return title_stripped

    # -- 비활성화된 키워드 치환 코드 (참고용, 사용 중단) --
    # 아래 PHRASE_MAP/WORD_MAP 방식은 한/영 혼합 문장(예: "Tech Download: 어떻게 러시아 could profit
    # 이란 전쟁 helium supply chain disruption chip sector")을 생성하여 원문보다 가독성이 낮아짐.
    # FULL_TITLE_MAP에 수동으로 번역을 추가하는 방식을 사용할 것.
    #
    # PHRASE_MAP = {
    #     "North Korea": "북한",
    #     "South Korea": "한국",
