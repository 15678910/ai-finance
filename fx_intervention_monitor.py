...
def main():
    ...
    out = {
        ...
        "portfolio_note": ("달러 기준 외국인의 환손익은 '원화가 강해질 때' 발생한다(보유 원화자산의 달러 환산액 증가). "
                           "따라서 매수 유인이 가장 큰 조합은 '지금은 저평가(싸게 진입) + 앞으로 강세 전환 기대'다 — "
                           "약세 그 자체가 유인이 아니라, 약세에서 강세로 돌아설 것이라는 기대가 유인이다. "
                           "반대로 엔 캐리가 청산되면 글로벌 위험자산이 함께 흔들린다. 두 경로 모두 국내 증시 수급에 "
                           "닿지만, 이 지표는 전환 시점을 알려주지 않는다."),
