...
def generate(date_str: str, daily_dir: str, output_path: str) -> bool:
    ...
    # 10.6) 저평가 스크리너 결과 병합 (있는 경우)
    value_screener_path = os.path.join(os.path.dirname(output_path), "value_screener.json")
    if os.path.exists(value_screener_path):
        try:
            with open(value_screener_path, encoding="utf-8") as f:
                output_data["value_screener"] = json.load(f)
            print("[INFO] 저평가 스크리너 결과 병합 완료")
        except Exception as e:
    ...
    # 10.11) 비트코인 본위제 모니터링 결과
    bts_path = os.path.join(os.path.dirname(output_path), "bitcoin_standard.json")
    if os.path.exists(bts_path):
        try:
            with open(bts_path, encoding="utf-8") as f:
                output_data["bitcoin_standard"] = json.load(f)
            print("[INFO] 비트코인 본위제 데이터 병합 완료")
        except Exception as e:
    ...
    # 10.13g) 반도체 챌린저 모니터 (신흥 위협 분석)
    chal_path = os.path.join(os.path.dirname(output_path), "semi_challengers.json")
    if os.path.exists(chal_path):
        try:
            with open(chal_path, encoding="utf-8") as f:
                output_data["semi_challengers"] = json.load(f)
            print("[INFO] 반도체 챌린저 데이터 병합 완료")
        except Exception as e:
            print(f"[WARN] 반도체 챌린저 데이터 로드 실패: {e}")

    # 10.13h) 한국 시장 투자자 매매 동향 (외국인·기관 일별)
    inv_path = os.path.join(os.path.dirname(output_path), "investor_flow.json")
    if os.path.exists(inv_path):
        try:
            with open(inv_path, encoding="utf-8") as f:
                output_data["investor_flow"] = json.load(f)
            print("[INFO] 투자자 매매 동향 데이터 병합 완료")
        except Exception as e:
            print(f"[WARN] 투자자 매매 동향 데이터 로드 실패: {e}")

    # 10.13i) 장중 투자자 매매 모멘텀 (Phase B — 시간별 스냅샷)
    intra_path = os.path.join(os.path.dirname(output_path), "investor_flow_intraday.json")
    if os.path.exists(intra_path):
        try:
            with open(intra_path, encoding="utf-8") as f:
                output_data["investor_flow_intraday"] = json.load(f)
            print("[INFO] 장중 투자자 매매 모멘텀 데이터 병합 완료")
        except Exception as e:
    ...
    # 10.13k) SAO 통합 시그널 (Multi-Asset Fusion Center)
    sao_path = os.path.join(os.path.dirname(output_path), "sao_signals.json")
    if os.path.exists(sao_path):
        try:
            with open(sao_path, encoding="utf-8") as f:
                output_data["sao_signals"] = json.load(f)
            print("[INFO] SAO 통합 시그널 병합 완료")
        except Exception as e:
