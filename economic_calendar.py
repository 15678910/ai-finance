...
CURATED_EVENTS = [
    ...
    {
        "date": "2026-08-TBD",
        "title": "SK하이닉스 미국 ADR 상장 (NYSE)",
        "category": "기업",
        "impact": "HIGH",
        "region": "🇰🇷🇺🇸",
        "detail": ("NYSE Level-3 ADR — '자사주 매각'이 아니라 '신주발행(유상증자)' 방식 확정. "
                   "신주 ₩10~15조 + 자사주 1,530만주 소각 병행. SEC 승인 임박(6/22주)→7~8월 데뷔. 장외 HXSCL."),
        "tags": ["SK하이닉스", "ADR", "유상증자", "오버행", "HBM", "상장"],
        "impact_analysis": (
            "★구조가 관건: 자사주 매각(비희석)이 아니라 신주발행(유상증자=희석) → 기존주주 지분 희석. "
            "단 자사주 1,530만주 소각을 병행해 희석 부분 상쇄(순효과는 신주규모:소각 비율이 좌우). "
            "🔴 핵심 리스크=오버행·'역유입': 신주가 국내 예탁→시장 유입 가능, ADR가격<원주면 차익전환 매도로 KOSPI주 하방압력. "
            "🟢 재평가 여지: SK 선행 PER 5.4배 vs 마이크론 19배 구조적 저평가 + ADR 美 장중 거래로 디커플링 완화. "
            "결국 신주발행 규모·일정 + 메모리 가격 방향이 분기점."
        ),
    },
