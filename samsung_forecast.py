"""
삼성전자 OHLC 예측기 — 개장 전 고정 + 마감 후 채점 (누적 성적표)
====================================================================
SK하이닉스 예측기와 동일 철학·엔진(반도체 빅섹터 완성):
  · 개장 전(07:00 KST, 미국 마감 직후) 나스닥선물 야간신호로 다음 거래일 OHLC 예측
  · 한 번 고정하면 장중 수정 금지(freeze) — today 블록 persist
  · 마감 후 실제 종가로 워크포워드 채점(해당일 제외) → 종가 MAE·방향·범위 적중 누적

엔진: 나스닥선물(NQ=F) 24H 야간(직전 마감 06:30 UTC → 07:00 KST) × 워크포워드 β.
참고: 마이크론·브로드컴·SOXX·엔비디아 + 아시아 반도체 동조.

출력: docs/samsung_forecast.json
...
def _round_tick(p):
    """KRX 호가단위(2023 개정): 가격대별 차등. 삼성(20만~50만)=500원, 50만↑=1000원."""
    if p >= 500000:
        u = 1000
    elif p >= 200000:
        u = 500
...
def main():
    ...
    print(f"삼성전자 최근 {last_date}: 시{lo_o:.0f} 고{lo_h:.0f} 저{lo_l:.0f} 종{last_close:.0f}")
    ...
    # 역사 표본: 직전마감(06:30 UTC) → 07:00 KST(=D 00:00 UTC-2h) 선물 야간 + 실제 OHLC
    recs = []
    ...
    def betas(win):
        ...
        # [v1.2] 디커플링 방향 보정: 창 내 선물↔종가 상관 r²이 낮으면 베타를 flat 쪽으로 수축.
        try:
    ...
    if hm < CLOSE_HM:
        if prev_today.get("status") == "locked" and prev_today.get("target_date") == today_str:
            ...
            print(f"  ▶ {today_str} 잠정(07:00 확정 전): 종가 {today['close']:.0f}")
    else:
        today = compute_today("provisional", "다음 거래일")
        print(f"  ▶ 다음 거래일 잠정(내일 07:00 확정 예정): 종가 {today['close']:.0f}")

    # 이벤트 영향권 경고(FOMC/BOJ ±1거래일)
    try:
    ...
    # 방향 신뢰도(B안) — 美 반도체 야간신호 강도(±1%↑) + SOXX·나스닥선물 방향 일치
    try:
    ...
    output = {
        ...
        "note": ("개장 전(07:00 KST) 나스닥선물 야간신호로 다음 거래일 OHLC를 고정 → 장중 수정 없음 → "
                 "마감 후 실제 종가로 워크포워드 채점. 범위 적중=실제 종가가 예측 고저 안에 들어온 비율. "
                 "SK하이닉스와 함께 반도체 빅섹터 양대 축. 통계 추정."),
    }
